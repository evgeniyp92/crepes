...
The request data holds what was sent but Express cant access it by default so we need middleware

add `app.use(express.json())` to get express to receive the body
